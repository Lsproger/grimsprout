...
| `/revert <short_sha> [--hard]` | editor+ | безопасный откат bot-коммита по короткому SHA |
...
## 3.4. Сценарий: откат ошибочного коммита

```mermaid
sequenceDiagram
    autonumber
    User->>Bot: /revert a1b2c3d4
    Bot->>Auth: проверка роли
    Auth-->>Bot: editor (ok)
    Bot->>Git: find_commit_by_short_sha(work_branch, a1b2c3d4)
    Git-->>Bot: bot commit SHA / ошибка
    alt confirm_commits=true
        Bot-->>User: превью отката + кнопки Подтвердить/Отменить
        User->>Bot: Подтвердить
    end
    Bot->>Git: revert_commit(target_sha)
    Git-->>Bot: revert_sha
    Bot->>Audit: log(action=revert, original_sha, revert_sha)
    Bot-->>User: "↩️ Откат выполнен"
```

- `/revert <short_sha>` ищет коммит по префиксу SHA только в `repository.work_branch`.
- В обычном режиме доступны только последние 20 bot-коммитов с маркером `GrimSprout: tg_id=...`.
- `--hard` снимает ограничение по окну поиска, но доступен только `admin`.
- Откат реализуется только через `git revert`, то есть создаётся новый коммит, а история не переписывается.
- Если `confirm_commits=true`, откат проходит через отдельное подтверждение перед выполнением.

## 3.5. Сценарий: напоминание и регрессия
...
- **`/revert`: short SHA не найден / неоднозначен / не bot-коммит** → откат не выполняется, бот возвращает диагностическое сообщение без изменения репозитория.
